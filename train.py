...
#input_ids = np.memmap(os.path.join(data_dir, 'input_ids.bin'), dtype=np.uint16, mode='r')
#output_embeddings = np.memmap(os.path.join(data_dir, 'output_embeddings.bin'), dtype=np.float32, mode='r')
# just load them in memory
...
def get_batch(split):
    ...
    y = torch.stack([torch.from_numpy((output_embeddings[i]).astype(np.float32)) for i in ix])
...
if init_from == 'scratch':
    ...
    model_args['vocab_size'] = 320 # the largest value is 258, rounded up to multiples of 64
